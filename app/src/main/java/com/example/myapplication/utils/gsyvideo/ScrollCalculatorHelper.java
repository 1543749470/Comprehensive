...
public class ScrollCalculatorHelper {
    ...
    void playVideo(RecyclerView view) {
        ...
        GsyVideo gsyBaseVideoPlayer = null;

        boolean needPlay = false;

        for (int i = 0; i < visibleCount; i++) {
            if (layoutManager.getChildAt(i) != null && layoutManager.getChildAt(i).findViewById(playId) != null) {
                GsyVideo player = (GsyVideo) layoutManager.getChildAt(i).findViewById(playId);
                Rect rect = new Rect();
        ...
        if (gsyBaseVideoPlayer!=null){
            gsyBaseVideoPlayer.changeUiToPreparingShow();
        }

        if (gsyBaseVideoPlayer != null && needPlay) {
            if (runnable != null) {
                GsyVideo tmpPlayer = runnable.gsyBaseVideoPlayer;
    ...
    private class PlayRunnable implements Runnable {
        ...
        GsyVideo gsyBaseVideoPlayer;

        public PlayRunnable(GsyVideo gsyBaseVideoPlayer) {
    ...
    /***************************************自动播放的点击播放确认******************************************/
    private void startPlayLogic(GsyVideo gsyBaseVideoPlayer, Context context) {
    ...
    private void showWifiDialog(final GsyVideo gsyBaseVideoPlayer, Context context) {
