...
public class LogInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        String url = request.url().toString() + "?sendTime=" + System.currentTimeMillis();
        if (logRequest) {
            EventBusUtils.INSTANCE.sendEvent(new GetNetworkEvent(url));
            //打印请求信息
...
//            WriteLogFile.INSTANCE.writeFile(url, bodyString, null);
            if (responseBody != null && isParseable(responseBody.contentType())) {
